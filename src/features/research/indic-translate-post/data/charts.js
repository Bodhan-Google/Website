...
import { SENT_JUDGE_MACRO, SENT_JUDGE_PERFECT } from './sentenceJudge.js';
import { TRANSLIT_CER_WER } from './translitMetrics.js';
...
// WER replaced dchrF++ in this tile. It is an error rate, so the bar order is
// reversed relative to dBLEU -- lowest first -- and the ranking is still read
// top-down as best-to-worst.
export const docWer = {
  type: 'rankedBar',
  id: 'doc-wer',
  title: 'Word error rate',
  subtitle: 'One document per unit. Lower is better.',
  description:
    'Document-level word error rate over 22 languages. Indic-Translate has the lower '
    + 'error rate than both baselines in all 22 of 22 languages.',
  valueHeader: 'WER',
  digits: 4,
  highlightKey: 'ours',
  data: ['ours', 'sarvam', 'it2'].map((k) => ({
...
export const docJudgeMean = {
  type: 'rankedBar',
  id: 'doc-judge-mean',
  title: 'Document Translation Quality, mean overall score',
  subtitle: 'LLM Judge rubric, 0–100. Higher is better.',
  description:
    'Mean score of 82.49 for Indic-Translate against 63.99 for Sarvam Translate.',
...
export const docJudgeDims = {
  ...
  id: 'doc-judge-dims',
  maxValue: 5,
  title: 'Document Translation Quality by dimension',
  subtitle:
    'LLM Judge rubric, 0–5 per dimension, against Sarvam Translate. Higher is better.',
  description:
    'Grouped bars over six rubric dimensions. Indic-Translate scores higher on every one.',
  // The judge models are named in the generated data files' provenance headers, not
  // here: the page states the method, not the vendor.
  note:
    'Scored by an LLM Judge and cross-checked with a second one; both rank this '
    + 'checkpoint ahead of Sarvam Translate on every dimension.',
...
export const docHumanDims = {
  ...
  subtitle: 'Same six-dimension rubric, scored by human annotators rather than an LLM Judge.',
...
// Our model only. The sentence tier is saturated -- 4.90 here against the previous
// checkpoint's 4.89 -- so a second series would draw a gap that is not there. One
// series, three rubric dimensions as the categories.
//
// `bar` (GroupedBars) rather than `rankedBar` for one reason: it is the only type whose
// XAxis honours `maxValue`. rankedBar hardcodes `domain={[0, 'auto']}`, and recharts
// then picked ticks 0/2/4/6/8 for this data -- a 1-5 rubric drawn on a 0-8 axis, which
// reads as a score out of 10. maxValue: 5 pins the axis at 0-5, and the subtitle,
// the table header and the note all say "out of 5" so the scale never rests on the
// axis alone. Verify the rendered ticks after any change here.
export const sentJudgeDims = {
  type: 'bar',
  id: 'sent-judge-dims',
  maxValue: 5,
  title: 'Sentence quality by dimension',
  subtitle:
    'LLM Judge rubric, out of 5. English → Indic, all 22 languages. Higher is better.',
  description:
    `An LLM Judge scored English → Indic sentences out of 5 on three dimensions: `
    + `adequacy ${SENT_JUDGE_MACRO.adequacy.toFixed(2)}, `
    + `fluency ${SENT_JUDGE_MACRO.fluency.toFixed(2)}, `
    + `overall ${SENT_JUDGE_MACRO.overall.toFixed(2)}. `
    + `${SENT_JUDGE_PERFECT.pct}% of sentences took the top score on all three.`,
  note:
    `${SENT_JUDGE_PERFECT.pct}% of sentences scored 5 out of 5 on all three dimensions. `
    + 'Reference-free LLM Judge scoring over roughly a thousand IN22-Gen sentences in '
 + 'each of the 22 languages. English → Indic only.',
  categoryHeader: 'Dimension',
  yLabel: 'Score out of 5',
  digits: 2,
  // Keyed 'ours' so the single series is never faded on hover, and so it takes the
  // house primary colour like every other Indic-Translate series on the page.
  series: [{ key: 'ours', label: OURS, role: 'primary' }],
  data: [
    { category: 'Adequacy', ours: SENT_JUDGE_MACRO.adequacy },
    { category: 'Fluency', ours: SENT_JUDGE_MACRO.fluency },
    { category: 'Overall', ours: SENT_JUDGE_MACRO.overall },
  ],
};
...
export const chartRegistry = Object.fromEntries(
  [
    // dBLEU first, then WER -- the order they appear in the document tile.
    docDbleu,
    docWer,
    docJudgeMean,
    docJudgeDims,
